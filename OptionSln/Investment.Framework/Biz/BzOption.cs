            ...
            try
            {
                using (StocksDbDataContext db = new StocksDbDataContext())
                {
                    ...
                    using (StreamWriter sw = new StreamWriter(ConfigurationManager.AppSettings["OutputLocation"] + "data.js"))
                    {
                        ...
                        {
                            olist[i].年化收益率 = decimal.Parse(((olist[i].资产 - olist[i].本金) / olist[i].本金 / ((olist[i].recordtime - startDate).Days + 1) * 36500).ToString("f4"));
                        }
                        ...
                        {
                            weeklist[i].annualrate = decimal.Parse(((weeklist[i].asset - 1) / (weeklist.Count - i + startWeek) * 5200).ToString("f4"));
                        ...
                        {
                            monthlist[i].annualrate = decimal.Parse(((monthlist[i].asset - 1) / (monthlist.Count - i) * 1200).ToString("f4"));
